...
const SearchInput = styled.input`
  padding: 13px 13px 13px 40px; /* Increased left padding to accommodate icon */
  ...
  @media (max-width: 768px) {
    padding-left: 40px; /* Consistent padding for icon */
  }
  
  @media (max-width: 480px) {
    padding-left: 35px; /* Slightly reduced for smaller screens */
  }
`;
...
const SearchIcon = styled.svg`
  ...
  @media (max-width: 768px) {
    left: 12px; /* Keep icon inside input on mobile */
  }
...
const StyledDescription = styled(RequestDescription)`
  ...
  white-space: nowrap;
  position: relative;
  content: "Из 1 233 333 специалистов обязательно найдется тот кто поможет";
...
export default function BannerClient({ initialSlide = 0 }: BannerClientProps) {
  ...
  const topSpecialties = useMemo(
    () => [
      { name: t("topSpecialties.tutors"), link: "/search?category=math" },
      { name: t("topSpecialties.repair"), link: "/search?category=english" },
      {
        name: t("topSpecialties.construction"),
        link: "/search?category=plumber",
      },
      { name: t("topSpecialties.makeup"), link: "/search?category=tire" },
    ],
    [t]
  );
  ...
  // Replace the fetchData useEffect in BannerClient with this:
 ...
 useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      ...
      // Set empty reklama data since endpoint doesn't exist yet
      // When backend implements /api/reklamas/, uncomment the fetch below
      setReklamaData([]);
      
      /* Uncomment this when backend implements reklamas endpoint:
      try {
        const reklamasData = await apiClient.getReklamas();
        if (Array.isArray(reklamasData)) {
          setReklamaData(reklamasData);
        } else if (reklamasData && 'results' in reklamasData) {
          setReklamaData((reklamasData as any).results || []);
        } else {
          setReklamaData([]);
        }
      } catch (reklamaError) {
        console.log("Reklamas not available");
        setReklamaData([]);
      }
      */

      // Set categories with proper type handling
      ...
      // Set subcategories with proper type handling
  ...
  const handleSubCategoryClick = (subCategoryId: number) => {
    router.push(`/search?subcategory=${subCategoryId}`);
  };

  const handleShowAllClick = (categoryId: number) => {
    router.push(`/search?category=${categoryId}&all=true`);
  };
